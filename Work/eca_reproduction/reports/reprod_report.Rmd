...
* Total catch used in StoX-Reca estimate was 1198 t

I did not check, but assume this discrepancy is due to difference in landings data versions from Fdir.
...
#### ICES area IV
Parameters extracted from ECA 3.x .param files suggest the model was configured with season and gear as fixed effects, but I did not have enough samples with weight in the data compiled for reproduction to configure it like that. I configured the temporal covariate as a random effect in stead.

* Total catch used in ECA 3.x estimate was 5133 t
* Total catch used in StoX-Reca estimate was 4456 t

This difference seems to be mostly due to different version of Fdir landing file. I summed all cod over a landings file with version date close in time to when Åge run the NSSK estimates. The total weight of cod in ICES are IV, not in coastal regions were 5197 kt.

Running this configuration triggered a bug in the RECA library. This will be reported to the Norwegian Computing Centre and Estimates will be revisited when a revised library is delivered.
