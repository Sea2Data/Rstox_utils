...
#'
StationLengthDist <- function(baselineOutput, BioticData="FilterBiotic", LengthDistType="PercentLengthDist", allowMissingWeight=TRUE){
	...
	getStationLengthDist(BioticDataList, LengthDistType=LengthDistType, allowMissingWeight=allowMissingWeight)
}
#'
#' @import data.table
#' @export
#' @rdname StationLengthDist
#'
getStationLengthDist <- function(BioticDataList, LengthDistType="PercentLengthDist", allowMissingWeight=TRUE){
...
# New code to be included in the RstoxFramework package:
...
#' @param LengthDistType		The type of length distribution to use, one of "LengthDist", "NormLengthDist" and "PercentLengthDist" (see 'Details').
#' @param allowMissingWeight	Logical: If TRUE and \code{LengthDistType} == "PercentLengthDist" accept stations with missing pairs of lengthsampleweight and (total) weight or lengthsamplecatch and (total) catch.
#' @param BioticData		A list of biotic data representing the levels fishstation, catchsample and individual as returned from e.g. getBaseline("Test_Rstox")$outputData$ReadBioticXML.
#'
#' @details The purpose of function StationLengthDist is to produce a length frequency distribution for each biotic station by species. Three different distributions (LengthDistType) can be generated:
#' 
#' @return
...
#' 
#' @examples
#' dat <- getBaseline("Test_Rstox", input=FALSE, proc="ReadBioticXML")
...
StationLengthDist <- function(
    ...
    # Define the names of the required tables:
